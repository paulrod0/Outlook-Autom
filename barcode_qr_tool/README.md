...
- **Exportación**: ZIP de imágenes (PNG) o **hoja PDF imprimible** en rejilla.
...
# Hoja PDF imprimible de códigos de barras Code 128
python -m barcode_qr_tool.cli batch refs.csv --column 0 \
    --mode barcode --symbology code128 --pdf --out hoja.pdf
...
`--no-text`, `--qr-error`) disponibles en ambos subcomandos.
...
├── generator.py      # Núcleo: validación, render y exportación (ZIP/PDF)
├── requirements.txt
...
- Logo central en los QR (con corrección de error H).
- Plantillas de etiquetas predefinidas (Avery, Dymo).
- Exportación SVG vectorial para impresión a cualquier escala.
